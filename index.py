...
def push_server(sckey, desp=""):
    ...
    if sckey == '':
        ...
        if json_data['code'] == 0:
            print("推送成功。")#print(f"[{now}] 推送成功。")
        else:
             print("推送SB。")#print(f"[{now}] 推送失败：{json_data['code']}({json_data['message']})")
...
def push_tg(token, chat_id, desp=""):
    ...
    if token == '':
        ...
        server_url = "https://api.telegram.org/bot"+token+"/sendmessage"
        params = {
            "text": '小米运动 步数修改\n\n' + desp,
            "chat_id": chat_id
        }
        ...
        if json_data['ok'] == True:
            print("推送成功")#print(f"[{now}] 推送成功。")
               ...
               print("推送SB")#print( f"[{now}] 推送失败：{json_data['error_code']}({json_data['description']})")
# 企业微信推送
...
def wxpush(msg, usr, corpid, corpsecret):
    ...
    def send_message(msg, usr):
        ...
        if ret["errcode"] == 0:
            ...
            print("[+""now+] 推送失败：{ret['errcode']} 错误信息：{ret['errmsg']}")
